...
$pageTitle = "Inscrire un Nouvel Enseignant";
$message = '';
$message_type = '';

// Chemin pour les uploads de photos
define('UPLOAD_DIR_ENSEIGNANT', dirname(dirname(dirname(__DIR__))) . '/assets/uploads/avatars/');
define('UPLOAD_URL_DIR_ENSEIGNANT', BASE_URL . 'assets/uploads/avatars/');

if (!is_dir(UPLOAD_DIR_ENSEIGNANT)) {
    if (!mkdir(UPLOAD_DIR_ENSEIGNANT, 0775, true)) {
        error_log("Erreur: Impossible de créer le dossier d'upload: " . UPLOAD_DIR_ENSEIGNANT);
    }
}


if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['inscrire_enseignant'])) {
    ...
    // Validations
    if (empty($nom) || empty($prenom) || !$email || empty($mot_de_passe) || empty($specialite)) {
        ...
        try {
            ...
            // 2. Gérer l'upload de la photo
             if (isset($_FILES['photo']) && $_FILES['photo']['error'] == UPLOAD_ERR_OK) {
                if (!is_writable(UPLOAD_DIR_ENSEIGNANT)) {
                     throw new Exception("Le dossier d'upload n'est pas accessible en écriture : " . UPLOAD_DIR_ENSEIGNANT);
                }
                $tmp_name = $_FILES['photo']['tmp_name'];
                $file_name = time() . '_' . basename($_FILES['photo']['name']);
                $destination = UPLOAD_DIR_ENSEIGNANT . $file_name;
            ...
            $stmt_user = $database->prepare("INSERT INTO utilisateurs (nom, prenom, email, mot_de_passe, role, photo) 
                                           VALUES (:nom, :prenom, :email, :mdp, 'enseignant', :photo)");
            ...
            // 4. Créer l'entrée enseignant
            $stmt_enseignant = $database->prepare("INSERT INTO enseignants (id_utilisateur, specialite) 
                                                 VALUES (:id_user, :specialite)");
            $database->execute($stmt_enseignant, [
                ':id_user' => $id_utilisateur_cree, 
                ':specialite' => $specialite
            ]);

            $db->commit(); // Valider la transaction
            $_SESSION['message'] = "Enseignant inscrit avec succès! (Utilisateur ID: $id_utilisateur_cree)";
            $_SESSION['message_type'] = "success";

        } catch (Exception $e) {
            $db->rollBack(); // Annuler la transaction
            $_SESSION['message'] = "Erreur lors de l'inscription de l'enseignant: " . $e->getMessage();
            $_SESSION['message_type'] = "danger";
             if ($photo_path && file_exists(UPLOAD_DIR_ENSEIGNANT . $photo_path)) {
                unlink(UPLOAD_DIR_ENSEIGNANT . $photo_path);
            }
        }
    }
    header("Location: inscrire_enseignant.php");
    exit();
}
    ...
    <div class="main-wrapper">
        ...
        <main class="content-area">
            ...
            <div class="card card-custom">
                <div class="card-header"><i class="fas fa-user-plus"></i> Formulaire d'Inscription Enseignant</div>
                <div class="card-body">
                    <form action="inscrire_enseignant.php" method="POST" enctype="multipart/form-data">
                        ...
                        <hr>
                        <h5 class="mb-3 mt-4">Informations Enseignant</h5>
                        <div class="mb-3">
                        ...
                        <div class="d-flex justify-content-end mt-3">
                            <button type="submit" name="inscrire_enseignant" class="btn btn-custom-primary"><i class="fas fa-user-check"></i> Inscrire l'Enseignant</button>
                        </div>
                    </form>
                </div>
            </div>
